...
function Download-ExactUrl(
    ...
    if ($actual -ne $ExpectedSha256) {
        Remove-Item -LiteralPath $Destination -Force -ErrorAction SilentlyContinue
        throw "RECOVERY_RUNTIME_DOWNLOAD_HASH_MISMATCH:$(Split-Path -Leaf $Destination)"
    }
}

function Test-RuntimeReady([string]$Family, [string]$ExpectedVersion, [string]$WorkerName) {
    try {
        ...
        $current = Get-Content -LiteralPath $currentPath -Raw -Encoding UTF8 | ConvertFrom-Json -ErrorAction Stop
        if (
            [string]$current.schema -ne "tda_asr_runtime_v1" -or
            [string]$current.version -ne $ExpectedVersion
        ) { return $false }
...
if ($Automated) {
    ...
    Write-Host "Automated mode: the physical workers will prepare missing pinned models as part of the four-profile suite." -ForegroundColor Green
} else {
...
if ($Automated) {
    $ready = @(Get-ReadyProfiles $Port)
    $missing = @($RequiredProfiles | Where-Object { $_ -notin $ready })
    if ($missing.Count -ne 0) {
        throw "RECOVERY_AUTOMATED_PROFILES_NOT_READY:$($missing -join ',')"
    }
    Write-Host "All four ASR profiles are ready after automated physical preparation." -ForegroundColor Green
}
...
Write-Host "Do not publish transcripts or audio. Only the two sanitized final receipts are promotion evidence."
        ...
        return (Get-Sha256 $workerPath) -eq [string]$marker.worker_sha256
    } catch {
        return $false
    }
}

function New-NoCompressionZip([string]$Root, [string]$Destination) {
    ...
    try {
        ...
        try {
            foreach ($file in Get-ChildItem -LiteralPath $Root -File | Sort-Object Name) {
                $entry = $zip.CreateEntry($file.Name, [IO.Compression.CompressionLevel]::NoCompression)
                $out = $entry.Open()
                try {
...
function Install-RcRuntimeArtifact(
    [string]$Executable,
    [string]$Family,
    [string]$Artifact,
    ...
    $arguments = @(
        "--install-rc-runtime", $Family,
        "--rc-artifact", ('"{0}"' -f $Artifact),
        "--rc-artifact-sha256", $ArtifactSha256,
        "--rc-result-file", ('"{0}"' -f $ResultPath)
    )
    ...
    if (
        ...
        [string]$result.runtime -ne $Family -or
        [string]$result.status -ne "ready"
    ) {
...
function Ensure-AutomatedRuntimesReady([string]$DownloadRoot) {
    ...
    if (-not (Test-RuntimeReady "whisper" "1.1.4" "TDAWhisperWorker.exe")) {
        $whisperArtifact = Join-Path $runtimeAssets $WhisperRuntimeName
        Download-ExactUrl "https://github.com/Faysk/tda/releases/download/$WhisperRuntimeTag/$WhisperRuntimeName" $WhisperRuntimeSha256 $whisperArtifact
        Install-RcRuntimeArtifact $executable "whisper" $whisperArtifact $WhisperRuntimeSha256 (Join-Path $runtimeAssets "whisper-install.json")
        if (-not (Test-RuntimeReady "whisper" "1.1.4" "TDAWhisperWorker.exe")) {
            throw "RECOVERY_WHISPER_RUNTIME_NOT_READY"
        }
    }

    if (-not (Test-RuntimeReady "qwen" "1.0.7" "TDAQwenWorker.exe")) {
        ...
        $qwenArtifact = Join-Path $runtimeAssets "qwen-runtime-artifact.zip"
        New-NoCompressionZip $qwenRoot $qwenArtifact
        $qwenArtifactSha = Get-Sha256 $qwenArtifact
        Install-RcRuntimeArtifact $executable "qwen" $qwenArtifact $qwenArtifactSha (Join-Path $runtimeAssets "qwen-install.json")
        if (-not (Test-RuntimeReady "qwen" "1.0.7" "TDAQwenWorker.exe")) {
            throw "RECOVERY_QWEN_RUNTIME_NOT_READY"
        }
    }

    Write-Host "Exact Whisper 1.1.4 and Qwen 1.0.7 runtimes are ready." -ForegroundColor Green
}
