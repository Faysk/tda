...
function Ensure-AutomatedRuntimesReady([string]$DownloadRoot) {
    ...
    if (-not (Test-RuntimeReady "whisper" "1.1.4" "whisper-ctranslate2" "TDAWhisperWorker.exe")) {
        ...
        $whisperArtifact = Join-Path $runtimeAssets "whisper-actions-artifact.zip"
        New-NoCompressionZip $whisperRoot $whisperArtifact @($WhisperRuntimeName, $WhisperRuntimeDigestName)
        Install-RcRuntimeArtifact $executable "whisper" $whisperArtifact (Get-Sha256 $whisperArtifact) (Join-Path $runtimeAssets "whisper-install.json")

        if (-not (Test-RuntimeReady "whisper" "1.1.4" "whisper-ctranslate2" "TDAWhisperWorker.exe")) {
            throw "RECOVERY_WHISPER_RUNTIME_NOT_READY"
        }
    } else {
        Write-Host "Whisper runtime 1.1.4 already verified; reuse." -ForegroundColor Green
    }
    ...
    if (-not (Test-RuntimeReady "qwen" "1.0.7" "qwen3-transformers" "TDAQwenWorker.exe")) {
        ...
        $qwenArtifact = Join-Path $runtimeAssets "qwen-actions-artifact.zip"
        New-NoCompressionZip $qwenRoot $qwenArtifact @($QwenBundleName, $QwenPart1Name, $QwenPart2Name)
        Install-RcRuntimeArtifact $executable "qwen" $qwenArtifact (Get-Sha256 $qwenArtifact) (Join-Path $runtimeAssets "qwen-install.json")

        if (-not (Test-RuntimeReady "qwen" "1.0.7" "qwen3-transformers" "TDAQwenWorker.exe")) {
            throw "RECOVERY_QWEN_RUNTIME_NOT_READY"
        }
    } else {
        Write-Host "Qwen runtime 1.0.7 already verified; reuse." -ForegroundColor Green
    }
}
