...
try {
    ...
    if ($currentVersion -eq $ExpectedStableVersion -and (Test-InstalledPayload $candidate $payload)) {
        ...
        if ($currentVersion -eq $ExpectedStableVersion) {
            ...
            if ([string]$metadata.Version -ne $ExpectedStableVersion) { Fail "SUPERSEDED_VERSION_MISMATCH" }
    ...
    $whisperRuntime = Assert-RuntimeCurrent "whisper" $ExpectedWhisperRuntimeVersion "whisper-ctranslate2" ([string]$whisperExpected.archive_sha256)
    $qwenRuntime = Assert-RuntimeCurrent "qwen" $ExpectedQwenRuntimeVersion "qwen3-transformers" ([string]$qwenExpected.archive_sha256)
