...
function Ensure-Companion([object]$Candidate,[string]$Msi,[string]$Payload) {
  ...
  if($current -eq $version -and -not $exact) {
    ...
    if([string]$meta.Version -ne $version) { throw "SUPERSEDED_VERSION_MISMATCH" }
  ...
  $appRoot=Join-Path $env:LOCALAPPDATA "TDA\Companion\versions\$version"
  $exe=Join-Path $appRoot "TDACompanion.exe"
  $maintenance=Join-Path $appRoot "TDACompanionMaintenance.exe"
  Assert-Authenticode $exe
  Assert-Authenticode $maintenance
...
try{
  ...
  $summary=[ordered]@{
    ...
    expected_signer_subject=$(if($RequireAuthenticode){$ExpectedSignerSubject}else{$null})
...
if($passed){exit 0}
exit 1
) { throw "COMPANION_CANDIDATE_TAG_INVALID" }
  if (
    [string]$Candidate.schema -ne "tda_companion_candidate_v2" -or
    [string]$Candidate.channel -ne "rc" -or
    [string]$Candidate.tag -ne $Tag -or
...
exit 1
    ...
    '^companion-qwen-runtime-rc-v[0-9]+\.[0-9]+\.[0-9]+-[a-f0-9]{12}  if (
    [string]$Candidate.schema -ne "tda_runtime_candidate_v1" -or
    [string]$Candidate.family -ne $Family -or
    [string]$Candidate.runtime_id -ne $runtimeId -or
    [string]$Candidate.platform -ne "windows-x64" -or
...
if($passed){exit 0}
exit 1
) { throw "AUTHENTICODE_EXPECTED_THUMBPRINT_REQUIRED" }
  if([string]::IsNullOrWhiteSpace($ExpectedSignerSubject)) { throw "AUTHENTICODE_EXPECTED_SUBJECT_REQUIRED" }
  $sig=Get-AuthenticodeSignature -LiteralPath $Path
  if([string]$sig.Status -ne "Valid" -or $null -eq $sig.SignerCertificate) { throw "AUTHENTICODE_SIGNATURE_INVALID" }
  $actual=([string]$sig.SignerCertificate.Thumbprint -replace '\s','').ToUpperInvariant()
