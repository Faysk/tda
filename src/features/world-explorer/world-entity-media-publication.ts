...
function safeAssetForEntity(asset: AssetRow, entityId: string): boolean {
...
export async function publishWorldEntityMediaDraft({
	...
	if (campaignError || !campaign?.id) {
		console.error("World entity media publication campaign lookup failed", campaignError?.message);
		return "pending";
	}
	...
	for (const binding of bindings) {
		...
		try {
			...
			const { error: updateError } = await client
				...
				.eq("status", "staged");
			if (updateError) {
				console.error("World entity media verification persistence failed", updateError.message);
				return "pending";
			}
		} catch (error) {
			console.error(
