...
export function lembraExtensionForMime(
	mimeType: LembraMediaMime,
): LembraExtension {
...
export function lembraReferenceObjectKey({
	...
	return `lembra/${referenceId.toLowerCase()}/${sha256}.${extension}`;
}
...
export function validLembraAuthorName(value: unknown): value is string {
	return (
		typeof value === "string" &&
		value.trim().length >= 1 &&
		value.trim().length <= 120
