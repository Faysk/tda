...
export function lembraStagingBucket(): string {
	...
	const expected = production ? LEMBRA_PRIVATE_BUCKET : LEMBRA_PREVIEW_BUCKET;
...
async function putImmutable({
	...
	if (readBack.length !== info.bytes || lembraSha256(readBack) !== info.sha256) {
...
export async function readVerifiedLembraObject({
	...
	const allowedBuckets = new Set<string>([LEMBRA_PREVIEW_BUCKET, LEMBRA_PRIVATE_BUCKET]);
	if (!allowedBuckets.has(bucket)) failure("BUCKET_NOT_ALLOWED");
	...
	if (!expectedKey || objectKey !== expectedKey) failure("OBJECT_SCOPE_MISMATCH");
