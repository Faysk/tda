...
> Status: canônico para direção visual; migração da implementação em andamento
...
## Tokens canônicos alvo

Os tokens semânticos do pacote v1.0 são a direção oficial para o reboot.
...
## Estado atual do reboot

A implementação atual ainda contém tokens históricos simplificados (`--bg`, `--panel`, `--text`, `--gold`, `--line`) e Arial como corpo global em partes do frontend.
...
2. criar aliases temporários para consumidores antigos;
3. migrar componentes por domínio;
4. remover hex/tokens antigos somente quando não houver consumidores;
5. passar CI, desktop/mobile, light/dark, teclado e reduced motion;
...
3. Novo token precisa de papel definido, valores light/dark e verificação de contraste.
4. Componentes repetidos viram primitives compartilhadas.
5. Movimento comunica estado; não competir com narrativa.
6. Mobile reorganiza composição; não comprime desktop.
7. Marca não é redesenhada pelo Design System.
8. Tela não está pronta apenas porque parece boa em desktop dark.
