...
A Production CD usa o `VERCEL_TOKEN` protegido no GitHub Environment `production` para executar `vercel env pull --environment=production` somente quando o plano de release exige publicação de mídia. Desse env temporário, o job consome `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID` e `R2_SECRET_ACCESS_KEY`; `R2_PUBLIC_BUCKET` continua fixado no workflow. Esse é o mesmo boundary que já foi usado pelo publisher imutável de mídia antes da simplificação da esteira. O publisher canônico só roda quando o plano detecta alteração de mídia e publica os manifests alterados por `tools/ci/publish-production-media.sh`.
