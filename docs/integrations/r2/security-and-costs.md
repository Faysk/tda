...
Para upload direto do World, a implementação atual usa presigned `PUT` SigV4 de curta duração para uma chave única sob `uploads/pending/world-entity/`, atrás de `TDA_WORLD_ENTITY_MEDIA_ENABLED`. A assinatura vincula o `Content-Type`, mas não transforma MIME/hash/tamanho declarados pelo browser em evidência. O servidor reautoriza editor + lease, lê os bytes do R2, valida magic bytes + SHA-256 + tamanho + dimensões e só então registra o asset verificado; publicação/vínculo público continuam gates separados. O browser não recebe URL de escrita para `campaigns/.../portrait/{sha256}.{ext}`. Código/schema disponíveis não significam rollout remoto concluído.
...
Pricing revalidado em 2026-09-20 na documentação oficial (página atualizada em 2026-08-07): o free tier do **Standard** inclui 10 GB-mês, 1 milhão de operações Class A e 10 milhões de Class B por mês; egress para Internet permanece sem cobrança de bandwidth. O free tier não se aplica ao Infrequent Access. Conferir novamente antes de decisões: <https://developers.cloudflare.com/r2/pricing/>.
