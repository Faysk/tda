...
-- World entity media foundation. Browser roles receive no table or RPC grants.
-- Binary bytes live in Cloudflare R2; PostgreSQL owns stable identity, provenance,
-- verification state and the semantic entity -> media binding.
...
create table public.media_assets (
  ...
  check (object_key ~ '^campaigns/yuhara-main/entities/[0-9a-f-]{36}/(portrait|artwork|gallery)/[0-9a-f]{64}[.](png|webp)$'),
  check (
  ...
  loop
    ...
    if v_asset_id is null then
      ...
      insert into public.entity_media_bindings(
        ...
        v_campaign_id, v_entity_id, 'portrait', v_asset_id, 0.5, 0.5,
