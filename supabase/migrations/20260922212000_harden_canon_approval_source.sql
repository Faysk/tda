-- Harden the human canon approval boundary with a resolvable same-session source gate.
--
-- The original approval RPC already required explicit human authority. This
-- corrective migration additionally enforces the domain invariant "nothing becomes
-- canon without source" against physical evidence rows, not only declared UUID arrays.
...
begin
  ...
  if not exists (
       ...
       where segment.session_id = v_candidate.session_id
         and segment.id = any(coalesce(v_candidate.source_segment_ids, '{}'::uuid[]))
     )
     and not exists (
       select 1
       from public.roll20_events event
       where event.session_id = v_candidate.session_id
         and event.id = any(coalesce(v_candidate.source_roll20_event_ids, '{}'::uuid[]))
     ) then
...
comment on function public.approve_canon_candidate_atomic(uuid,uuid,text,uuid,text) is
  'Server-only human canon approval. Requires narrative.canon.approve plus a resolvable same-session transcript/Roll20 source, atomically records decision/audit and creates review_only canon; never publishes to a public audience.';
