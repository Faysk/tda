-- Production rollout for #430.
-- Immutable full transcript revisions, atomic current-pointer activation,
-- publication receipts/readback and lightweight publication events.
-- Browser roles remain denied; only the existing scoped site_editor role receives
-- campaign.transcript.publish. Existing role assignments and campaign scopes are preserved.
...
-- Deliberate rollout grant: Site Editor is the existing narrative role that can
-- open Edit and revise published campaign content. This adds the exact publish
-- capability to that role without creating or widening any user assignment.
do $$
declare
  v_role_id uuid;
begin
  if not exists (
    ...
    raise exception 'campaign.transcript.publish must exist before granting it';
  end if;
  ...
  values (v_role_id, 'campaign.transcript.publish')
