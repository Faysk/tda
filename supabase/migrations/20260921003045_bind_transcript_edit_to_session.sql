drop function if exists public.edit_transcript_segment_atomic(
  uuid, text, uuid, bigint, text, text, text, boolean, integer, integer
);
...
begin
  ...
  if p_expected_session_id is null then
    raise exception 'expected session is required' using errcode = '22023';
  end if;
  ...
  where ts.id = p_segment_id
    and ts.session_id = p_expected_session_id
...
comment on function public.edit_transcript_segment_atomic(
  uuid, text, uuid, uuid, bigint, text, text, text, boolean, integer, integer
)
is 'Server-only SECURITY INVOKER boundary for optimistic transcript edit + atomic audit. Campaign, expected session and segment identity are bound in the locked lookup before update.';

revoke all on function public.edit_transcript_segment_atomic(
  uuid, text, uuid, uuid, bigint, text, text, text, boolean, integer, integer
) from public;
revoke execute on function public.edit_transcript_segment_atomic(
  uuid, text, uuid, uuid, bigint, text, text, text, boolean, integer, integer
) from anon;
revoke execute on function public.edit_transcript_segment_atomic(
  uuid, text, uuid, uuid, bigint, text, text, text, boolean, integer, integer
) from authenticated;
grant execute on function public.edit_transcript_segment_atomic(
  uuid, text, uuid, uuid, bigint, text, text, text, boolean, integer, integer
) to service_role;
