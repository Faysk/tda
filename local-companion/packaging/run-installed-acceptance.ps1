...
function Ensure-AutomatedProfilesReady([string]$SourceId) {
    ...
    foreach ($profile in $profiles) {
        ...
        if ($operationId -notmatch '^[a-f0-9]{32}function Get-UiProcesses {
    $health = Get-AgentHealth
...
if ($Automated) {
    ...
    if ([string]$craigFirst.source_id -notmatch '^craig-[a-f0-9]{64}    $settingsOriginal = if ($settingsExisted) { Get-Content -LiteralPath $settingsPath -Raw -Encoding UTF8 } else { $null }
    ...
    try {
        ...
        Write-Host "[profile_preparation - automated]" -ForegroundColor Cyan
        Ensure-AutomatedProfilesReady ([string]$craigFirst.source_id)

        Write-Host "[craig_survives_agent_loss - automated]" -ForegroundColor Cyan
...
exit 0
        ...
        $completed = $false
        while ([DateTimeOffset]::UtcNow -lt $deadline -and -not $completed) {
            $status = Invoke-AgentJson "GET" "/preparation"
            if (
            ...
            switch ([string]$status.state) {
                ...
                "failed" {
                    ...
                    if ($code -notmatch '^[A-Z0-9_]{1,96}function Get-UiProcesses {
    $health = Get-AgentHealth
    $agentPid = if ($null -ne $health) { [int]$health.pid } else { -1 }
    return @(Get-CimInstance Win32_Process -Filter "Name='TDACompanion.exe'" -ErrorAction SilentlyContinue |
        Where-Object {
...
exit 0
        ...
        if (-not $completed) { throw "PROFILE_PREPARATION_TIMEOUT:$profile" }
...
Write-Host "BITS evidence sanitizada: $bitsEvidencePath"
exit 0
 -or [int]$craigFirst.track_count -lt 1) {
        throw "CRAIG_INGEST_INVALID"
    }
    $observations.Add("craig_selected")
