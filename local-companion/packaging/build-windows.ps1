...
if ($signingEnabled -and $normalizedThumbprint -notmatch '^[A-F0-9]{40}if (-not (Test-Path $pythonPath)) {
...
Write-Host ("Authenticode: " + ($(if ($signingEnabled) { "signed + verified" } else { "not configured" })))
if ($signingEnabled) {
...
function Assert-AuthenticodeIdentity([string]$Path) {
    ...
    if ([string]$signature.Status -ne "Valid" -or $null -eq $signature.SignerCertificate) {
        throw ("AUTHENTICODE_SIGNATURE_INVALID:" + [IO.Path]::GetFileName($Path) + ":" + [string]$signature.Status)
    }
