...
class Store:
    ...
    def action(self, job_id, action):
        with self.tx() as db:
            ...
            if action == "cancel":
                ...
                status = "queued"
            db.execute(
                "UPDATE jobs SET status=?,stage=?,error=NULL,updated=? WHERE id=?",
    ...
    def claim(self):
        with self.tx() as db:
            ...
            db.execute(
                "UPDATE jobs SET status='running',stage='fixture',attempt=?,updated=? WHERE id=?",
                (next_attempt, utc_now(), row["id"]),
            )
    ...
    def fail(self, job_id, attempt):
        with self.tx() as db:
            changed = db.execute(
                "UPDATE jobs SET status='failed',error='FIXTURE_EXECUTION_FAILED',updated=? WHERE id=? AND status='running' AND attempt=?",
